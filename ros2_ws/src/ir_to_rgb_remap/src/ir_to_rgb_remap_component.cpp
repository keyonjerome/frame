...
public:
  ...
  : rclcpp::Node("ir_to_rgb_remap", options) {
    in_image_ = this->declare_parameter<std::string>("in_image", "/camera/camera/infra1/image_rect_raw");
    out_image_ = this->declare_parameter<std::string>("out_image", "/image_rect");
    in_info_  = this->declare_parameter<std::string>("in_camera_info", "/camera/camera/infra1/camera_info");
...
private:
  void onImage(const sensor_msgs::msg::Image::SharedPtr msg) {
    try {
      // Force mono8 then convert to rgb8
