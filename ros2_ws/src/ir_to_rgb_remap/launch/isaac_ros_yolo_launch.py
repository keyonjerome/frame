...
def generate_launch_description():
    ...
    # 1) Collect all launch arguments & actions FIRST
    actions = list(IsaacROSYolov8LaunchFragment.get_launch_actions(ifs).values())

    # 2) Now create the container that consumes those LaunchConfigurations
    yolov8_container = ComposableNodeContainer(
        ...
        composable_node_descriptions=list(
            IsaacROSYolov8LaunchFragment.get_composable_nodes(ifs).values()
        ),
        arguments=['--ros-args', '--log-level', 'INFO'],
        output='screen'
    )

    # 3) Append the container after the declarations
    actions.append(yolov8_container)
